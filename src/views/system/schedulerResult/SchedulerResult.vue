  ...
  <div class="animated fadeIn">
    ...
    <section class="board">
      <b-table
        ...
        @row-clicked="details"
      >
      <template slot="successYn" slot-scope="row">{{row.value? '성공':'실패'}}</template>
      </b-table>
    </section>
...
<script>
  ...
  export default {
    ...
    data (){
      return {
        fields: {
          scheduleLogId: {label: 'ID'},
          scheduleName: {label: 'Scheduler Name', 'class': 'text-left'},
          createDateTime: {label: '처리일시'},
          successYn: {label: '처리결과'}
        },
        items: [],
        pageInfo: {
          page: 1,
    ...
    methods: {
      details (item) {
        this.$router.push({ name: 'Scheduler-Result 상세', params: { id: item.scheduleLogId }})
      },
      ...
      fetchList (params = {}){
        ...
        this.$https.get('/system/scheduleLogs', {...defaultParams, ...params})
          .then((res) => {
      ...
      onSearch (){
        ...
        Object.keys(this.searchItem).forEach(key => {
          if (this.searchItem[key] !== null && this.searchItem[key] !== ''){
            this.queryParams[key] = (key === 'successYn')
              ? (this.searchItem[key] === '성공')
              : this.searchItem[key];
          }
        });
      ...
      excelDownload(){
        ...
        return window.location.href = '/api/excel/scheduleLogs/download?q=' + q;
      },
    }
  }
</script>
