<template>
  <div class="animated fadeIn">
    ...
    <section class="board-btn">
      ...
      <b-button type="button" variant="primary" :to="{ name: 'Domain Processing - Pool', params: { method: 'edit' } }">
        Pool 수정
      </b-button>
      <b-button type="button" variant="primary" :to="{ name: 'Domain Processing - Pool', params: { method: 'create' } }">
        Pool 등록
      </b-button>
    </section>
...
<script>
  ...
  export default {
    name: 'domainProcessing',
    data (){
      return {
        fields: {
          processId: {label: 'PID'},
          processSectionCodeName: {label: 'Process', 'class': 'text-left'},
          taskNames: {label: 'Type', 'class': 'text-left'},
          createId: {label: '등록자'},
